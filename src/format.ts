...
function normalizeTaskCardFallbackText(value?: string): string | undefined {
  const trimmed = value?.trim();
  return trimmed ? trimmed : undefined;
}

function buildEffectiveTaskCardState(params: {
  ...
  const goalFallback =
    normalizeTaskCardFallbackText(params.taskState?.goal) ??
    normalizeTaskCardFallbackText(params.threadState?.threadName) ??
    normalizeTaskCardFallbackText(params.bindingThreadTitle);
  const stageFallback =
    params.taskState?.stage ??
    (params.planMode ? "planned" : params.bindingActive ? "executing" : "intake");
  const nextActionFallback =
    normalizeTaskCardFallbackText(params.taskState?.nextAction) ??
    (params.planMode
      ? "Review the current plan or send guidance"
  ...
  return {
    ...
    goal: goalFallback,
    stage: stageFallback,
    nextAction: nextActionFallback,
    updatedAt: params.taskState?.updatedAt ?? Date.now(),
  };
}

function appendTaskCardLines(lines: string[], taskState: TaskCardState | undefined): void {
  lines.push("", "Task card:");
  lines.push(`Stage: ${taskState?.stage?.trim() || "intake"}`);
  ...
  const lastHeartbeat = formatDateAge(taskState?.lastHeartbeatAt);
  if (lastHeartbeat) {
    lines.push(`Last heartbeat: ${lastHeartbeat}`);
  }
}
...
export function formatCodexStatusText(params: {
  ...
  const effectiveTaskState = buildEffectiveTaskCardState({
    ...
    bindingThreadTitle: params.bindingThreadTitle,
