...
describe("formatCodexStatusText", () => {
  ...
  it("renders the resume ledger and verified timestamp when task history exists", () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-04-06T12:30:00.000Z"));
    ...
    const text = formatCodexStatusText({
      ...
      taskState: {
        ...
        lastRunSummary: "Tests passed. Typecheck passed. Build passed.",
        resumeReason: "Send the next task message in this thread",
        lastFailure: "Previous attempt stalled on plugin reload",
        lastVerifiedAt: new Date("2026-04-06T12:29:00.000Z").getTime(),
        verification: {
          status: "verified",
          summary: "Tests passed. Typecheck passed. Build passed.",
          updatedAt: new Date("2026-04-06T12:29:00.000Z").getTime(),
        },
        updatedAt: new Date("2026-04-06T12:29:00.000Z").getTime(),
      },
    });

    expect(text).toContain("Resume ledger:");
    expect(text).toContain("Last run: Tests passed. Typecheck passed. Build passed.");
    expect(text).toContain("Last failure: Previous attempt stalled on plugin reload");
    expect(text).toContain("Resume reason: Send the next task message in this thread");
    expect(text).toContain("Last verified: 1m ago");
  });
